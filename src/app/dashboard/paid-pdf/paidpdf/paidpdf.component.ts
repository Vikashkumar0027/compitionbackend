...
@Component({
  selector: 'app-paidpdf',
  templateUrl: './paidpdf.component.html',
  styleUrl: './paidpdf.component.css'
})
export class PaidpdfComponent implements OnInit {
 bannerData: any[] = [];
  coursData: any[] = [];
  testOnlineData: any[] = [];
  searchCompany: any;
  ...
  constructor(
    ...
    private pdfService:PaidpdfService
  ) { }
  ...
  getcouseList() {
    try {
      ...
      this.courseService.courseList().subscribe(res => {
        console.log(res);
        if (res.success) {
          const dataPaid= res.response.filter((x:any)=>x.type == 'paid');
          this.coursData = dataPaid;
        }
        this.spinner.hide();
      }, err => {
        this.spinner.hide();
...
// "paid"
  ...
  getList() {
    try {
      ...
      this.pdfService.pdfList().subscribe(async res => {
        console.log(res);
  ...
  modalData() {
    this.activeModal = this.modalService.open(PaidpdfModalComponent, {
      size: 'lg',
      backdrop: 'static',
      keyboard: false,
    });
  ...
  async deletefunction(id: any) {
    ...
    this.pdfService.pdfDelete(id).subscribe(res => {
      // console.log(res);
      if (res.success) {
        this.getList();
